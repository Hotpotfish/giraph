...
/**
 * The Giraph-specific business logic for a single BSP compute node in whatever
 * underlying type of cluster our Giraph job will run on. Owning object will
 * provide the glue into the underlying cluster framework and will call this
 * object to perform Giraph work.
 * 
 * @param <I>
 *            Vertex id
 * @param <V>
...
public class GraphTaskManager<I extends WritableComparable, V extends Writable, E extends Writable, M extends Writable>
		implements ResetSuperstepMetricsObserver {
	/*
	 * if_not[PURE_YARN] static { // Eliminate this? Even MRv1 tasks should not
	 * need it here. Configuration.addDefaultResource("giraph-site.xml"); }
	 * end[PURE_YARN]
	 */
	...
	/** Superstep stats */
	private FinishedSuperstepStats finishedSuperstepStats = new FinishedSuperstepStats(
			0, false, 0, 0, false);
	 ...
	 * 
	 * @param context
	 *            a handle to the underlying cluster framework. For Hadoop
	 *            clusters, this is a Mapper#Context.
	 */
	 ...
	 * 
	 * @param zkPathList
	 *            the path to the ZK jars we need to run the job
	 */
	public void setup(Path[] zkPathList) throws IOException,
		...
		try {
			...
			if (zkManager != null) {
				zkManager
						.offlineZooKeeperServers(ZooKeeperManager.State.FAILED);
			}
	...
	/**
	 * Perform the work assigned to this compute node for this job run. 1) Run
	 * checkpoint per frequency policy. 2) For every vertex on this mapper, run
	 * the compute() function 3) Wait until all messaging is done. 4) Check if
	 * all vertices are done. If not goto 2). 5) Dump output.
	 */
	public void execute() throws IOException, InterruptedException {
		...
		WorkerAggregatorUsage aggregatorUsage = prepareAggregatorsAndGraphState();
		...
		do {
			...
			GiraphTimerContext superstepTimerContext = getTimerForThisSuperstep(superstep);
			GraphState<I, V, E, M> graphState = new GraphState<I, V, E, M>(
					superstep, finishedSuperstepStats.getVertexCount(),
					finishedSuperstepStats.getEdgeCount(), context, this, null,
					aggregatorUsage);
			Collection<? extends PartitionOwner> masterAssignedPartitionOwners = serviceWorker
			...
			context.progress();
			serviceWorker
					.exchangeVertexPartitions(masterAssignedPartitionOwners);
			context.progress();
			graphState = checkSuperstepRestarted(aggregatorUsage, superstep,
					graphState);
			prepareForSuperstep(graphState);
			context.progress();
			MessageStoreByPartition<I, M> messageStore = serviceWorker
					.getServerData().getCurrentMessageStore();
			int numPartitions = serviceWorker.getPartitionStore()
					.getNumPartitions();
			int numThreads = Math.min(numComputeThreads, numPartitions);
			if (LOG.isInfoEnabled()) {
				LOG.info("execute: " + numPartitions
						+ " partitions to process with " + numThreads
						+ " compute thread(s), originally " + numComputeThreads
						+ " thread(s) on superstep " + superstep);
			}
	 ...
	 * 
	 * @param im
	 *            the boolean input to set isMaster. Applies to "pure YARN only"
	 */
	 ...
	 * "finally" block of our GiraphYarnTask#execute() to commit final job
	 * output.
	 * 
	...
	/**
	 * Instantiate and configure ZooKeeperManager for this job. This will result
	 * in a Giraph-owned Zookeeper instance, a connection to an existing quorum
	 * as specified in the job configuration, or task failure
	 * 
	 * @return true if this task should terminate
	 */
	private boolean startZooKeeperManager() throws IOException,
			InterruptedException {
	 ...
	 * 
	 * @param aggregatorUsage
	 *            handle to aggregation metadata
	 */
	private void updateSuperstepGraphState(WorkerAggregatorUsage aggregatorUsage) {
		serviceWorker.getWorkerContext().setGraphState(
				new GraphState<I, V, E, M>(serviceWorker.getSuperstep(),
						...
						finishedSuperstepStats.getEdgeCount(), context, this,
						null, aggregatorUsage));
	}

	/**
	 * Utility function for boilerplate updates and cleanup done at the end of
	 * each superstep processing loop in the <code>execute</code> method.
	 * 
	 * @param partitionStatsList
	 *            list of stas for each superstep to append to
	 * @param superstepTimerContext
	 *            for job metrics
	 * @param graphState
	 *            the graph state metadata
	...
	private FinishedSuperstepStats completeSuperstepAndCollectStats(
		...
		finishedSuperstepStats = serviceWorker.finishSuperstep(graphState,
				partitionStatsList);
	...
	private WorkerAggregatorUsage prepareAggregatorsAndGraphState() {
		WorkerAggregatorUsage aggregatorUsage = serviceWorker
	 ...
	 * 
	 * @return an enum detailing the roles assigned to this compute node for
	 *         this Giraph job.
	 */
	...
	/**
	 * Set the concrete, user-defined choices about generic methods (validated
	 * earlier in GiraphRunner) into the Configuration.
	 * 
	 * @param conf
	 *            the Configuration object for this job run.
	 */
	public void determineClassTypes(Configuration conf) {
		ImmutableClassesGiraphConfiguration giraphConf = new ImmutableClassesGiraphConfiguration(
				conf);
		Class<? extends Vertex<I, V, E, M>> vertexClass = giraphConf
				.getVertexClass();
	 ...
	 * things like Oozie map-reduce jobs. NOTE: Pure YARN profile cannot make
	 * use of this, as the jars are unpacked at each container site.
	 * 
	 * @param myClass
	 *            Class to search the class loader path for to locate the
	 *            relevant jar file
	...
	private static String findContainingJar(Class<?> myClass) {
		...
		try {
			for (Enumeration<?> itr = loader.getResources(classFile); itr
					.hasMoreElements();) {
	 ...
	 * Basic logic is as follows: 1) If not split master, everyone does the
	 * everything and/or running ZooKeeper. 2) If split master/worker, masters
	 * also run ZooKeeper
	 * 
	 * 3) If split master/worker == true and <code>giraph.zkList</code> is set,
	 * the master will not instantiate a ZK instance, but will assume a quorum
	 * is already active on the cluster for Giraph to use.
	 * 
	 * @param conf
	 *            Configuration to use
	 * @param zkManager
	 *            ZooKeeper manager to help determine whether to run ZooKeeper.
	...
	/**
	 * Instantiate the appropriate BspService object (Master or Worker) for this
	 * compute node.
	 * 
	 * @param serverPortList
	 *            host:port list for connecting to ZK quorum
	 * @param sessionMsecTimeout
	...
	private void instantiateBspService(String serverPortList,
		...
		if (graphFunctions.isWorker()) {
			...
			serviceWorker = new BspServiceWorker<I, V, E, M>(serverPortList,
					sessionMsecTimeout, context, this);
			if (LOG.isInfoEnabled()) {
	...
	/**
	 * Attempt to locate the local copies of the ZK jar files, assuming the
	 * underlying cluster framework has provided them for us.
	 * 
	 * @param fileClassPaths
	 *            the path to the ZK jars on the local cluster.
	 */
	private void locateZookeeperClasspath(Path[] fileClassPaths)
		...
		if (!conf.getLocalTestMode()) {
			...
			if (fileClassPaths == null) {
				...
				zkClasspath = null == jarFile ? "./*" : jarFile.replaceFirst(
						"file:", "");
			} else {
	...
	private void initializeAndConfigureLogging() {
		...
		if (conf.useLogThreadLayout()) {
			PatternLayout layout = new PatternLayout(
					"%-7p %d [%t] %c %x - %m%n");
			Enumeration<Appender> appenderEnum = Logger.getRootLogger()
					.getAllAppenders();
	...
	public void newSuperstep(SuperstepMetricsRegistry superstepMetrics) {
		...
		computeAll = new GiraphTimer(superstepMetrics, TIMER_COMPUTE_ALL,
				TimeUnit.MILLISECONDS);
	...
	public void notifySentMessages() {
		// We are tracking the time between when the compute started and the
		// first
		// message get sent. We use null to flag that we have already recorded
		// it.
	 ...
	 *            the BSP graph state
	 * @param messageStore
	 *            the messages to be processed in this superstep
	 * @param numPartitions
	 *            the number of data partitions (vertices) to process
	 * @param numThreads
	 *            number of concurrent threads to do processing
	 */
	private void processGraphPartitions(
			final Mapper<?, ?, ?, ?>.Context context,
			List<PartitionStats> partitionStatsList,
			final GraphState<I, V, E, M> graphState,
			final MessageStoreByPartition<I, M> messageStore,
			int numPartitions, int numThreads) {
		final BlockingQueue<Integer> computePartitionIdQueue = new ArrayBlockingQueue<Integer>(
				numPartitions);
		for (Integer partitionId : serviceWorker.getPartitionStore()
				.getPartitionIds()) {
		...
		CallableFactory<Collection<PartitionStats>> callableFactory = new CallableFactory<Collection<PartitionStats>>() {
			...
			public Callable<Collection<PartitionStats>> newCallable(
					int callableId) {
				return new ComputeCallable<I, V, E, M>(context, graphState,
						messageStore, computePartitionIdQueue, conf,
						serviceWorker);
			}
		};
		List<Collection<PartitionStats>> results = ProgressableUtils
				.getResultsWithNCallables(callableFactory, numThreads,
						"compute-%d", context);
	...
	private GraphState<I, V, E, M> checkSuperstepRestarted(
		...
		if (serviceWorker.getRestartedSuperstep() == superstep) {
			...
			VertexEdgeCount vertexEdgeCount = serviceWorker
					.loadCheckpoint(serviceWorker.getRestartedSuperstep());
			finishedSuperstepStats = new FinishedSuperstepStats(0, false,
					vertexEdgeCount.getVertexCount(),
					vertexEdgeCount.getEdgeCount(), false);
	...
	/**
	 * Attempt to collect the final statistics on the graph data processed in
	 * this superstep by this compute node
	 * 
	 * @param inputSuperstepStats
	 *            the final graph data stats object for the input superstep
	 * @return true if the graph data has no vertices (error?) and this node
	 *         should terminate
	 */
	...
	/**
	 * Cleanup a ZooKeeper instance managed by this GiraphWorker upon job run
	 * failure.
	 */
	...
	/**
	 * Cleanup all of Giraph's framework-agnostic resources regardless of which
	 * type of cluster Giraph is running on.
	 */
	public void workerFailureCleanup() {
		try {
			...
			LOG.error(
					"run: Worker failure failed on another RuntimeException, "
