...
/**
 * Compute as many vertex partitions as possible. Every thread will has its own
 * instance of WorkerClientRequestProcessor to send requests. Note that the
 * partition ids are used in the partitionIdQueue rather than the actual
 ...
 *            Message data
 */
public class ComputeCallable<I extends WritableComparable, V extends Writable, E extends Writable, M extends Writable>
	...
	private WorkerClientRequestProcessor<I, V, E, M> workerClientRequestProcessor;
	...
	// o
	private static long thisVertex = 2;
	 ...
	 *            Context
	 * @param graphState
	 *            Current graph state (use to create own graph state)
	 * @param messageStore
	 *            Message store
	 * @param partitionIdQueue
	 *            Queue of partition ids (thread-safe)
	 * @param configuration
	 *            Configuration
	 * @param serviceWorker
	 *            Service worker
	...
	public ComputeCallable(Mapper<?, ?, ?, ?>.Context context,
		...
		// not long-lived, so just instantiating in the constructor is good
		// enough.
	...
	public Collection<PartitionStats> call() {
		...
		this.workerClientRequestProcessor = new NettyWorkerClientRequestProcessor<I, V, E, M>(
				context, configuration, serviceWorker);
		WorkerThreadAggregatorUsage aggregatorUsage = serviceWorker
		...
		this.graphState = new GraphState<I, V, E, M>(graphState.getSuperstep(),
				...
				graphState.getTotalNumEdges(), context,
				graphState.getGraphTaskManager(), workerClientRequestProcessor,
				aggregatorUsage);
		...
		while (!partitionIdQueue.isEmpty()) {
			...
			Partition<I, V, E, M> partition = serviceWorker.getPartitionStore()
					.getPartition(partitionId);
			try {
				PartitionStats partitionStats = computePartition(partition);
				partitionStatsList.add(partitionStats);
				long partitionMsgs = workerClientRequestProcessor
						.resetMessageCount();
				partitionStats.addMessagesSentCount(partitionMsgs);
				messagesSentCounter.inc(partitionMsgs);
				timedLogger.info("call: Completed " + partitionStatsList.size()
						+ " partitions, " + partitionIdQueue.size()
						+ " remaining " + MemoryUtils.getRuntimeMemoryStats());
			} catch (IOException e) {
				throw new IllegalStateException(
						"call: Caught unexpected IOException," + " failing.", e);
	...
	private PartitionStats computePartition(Partition<I, V, E, M> partition)
		...
		synchronized (partition) {
			...
			WorkerContext workerContext = graphState.getGraphTaskManager()
					.getWorkerContext();
			...
			// if (partitionStats.getFinishedVertexCount() ==
			...
			for (Vertex<I, V, E, M> vertex : partition) {
				...
				Iterable<M> messages = messageStore.getVertexMessages(vertex
						.getId());
